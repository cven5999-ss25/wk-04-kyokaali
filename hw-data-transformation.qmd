...
\<the combination of mutate () and case_when() creates a new column called income_cat in the data set composition_tidy_capita The code checks assigns values in the income_id column category labels such as HIC,UMC and so on.\>

3.  How is the object `levels_income_cat` used and what does it achieve?

\<The object `levels_income_cat` in the code defines the specific order of the income category levels. \>
...
\<I see a table with a column on income_id and the other with count. There are four rows and income id is described by income catergory, HIC,LIC.LMB abd UMC\>
...
\<With this second table, the income categories are organised from the highest to the lowest unlike the first one which was in no particular order\>
...
3.  Why do you see `NA` for mean_msw_total at "high income"? **Because there are missing values in the high income category so the mean wil be NA.**

4.  Remove the `#` at the row starting with `tidyr::`.

5.  Execute the code and observe the output in the Console. What has changed? How is that different to adding `na.rm = TRUE` as an argument to the `mean()` ? **Using drop_na() removes all rows with missing data before before grouping, so the mean will be a number. On the other hand na.rm =TRUE keeps all the rows in the data such that the count includes all the rows with NAs but the NAs are ignored during the computation of the mean.**
...
-   `gt()`:**converts the data to a table**
-   `tab_header()`:**adds title and sub title to the table**
-   `fmt_number()`:**formats numeric columns**
-   `starts_with()`:**selects columns by prefix**
-   `cols_labels()`:**labels columns in the table**
...
-   `rename()`:renames the column headings
-   `knitr::kable()`: will summarise the data in a table
